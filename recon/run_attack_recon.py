...
def find_models(output_dir):
    """البحث عن ملفات .keras في مجلد التدريب الخاص بـ recon"""
    import glob
...
def main():
    ...
    if args.target_domain:
        ...
        default_domain = cfg.TARGET_DOMAIN
        inp = input(f"Enter target domain [{default_domain}]: ").strip()
        target_domain = inp if inp else default_domain
    ...
    cfg.update_network_config(interface=interface, target_domain=target_domain)

    attack_type = cfg.DEFAULT_ATTACK
    ...
    current_state = np.zeros(cfg.STATE_SIZE, dtype=np.float32)

    for i in range(args.iterations):
        print(f"\n[*] Iteration {i+1}/{args.iterations}")
        best_params = agent.get_best_params(current_state)
        ...
        current_state = state_builder.build(alerts, result, 0, cfg.MAX_STEPS)
