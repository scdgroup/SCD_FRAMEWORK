...
class EvaluationReport:
    ...
    def _simplify_params(self, params_json_str):
        """تحويل JSON string للمعلمات إلى نص مقروء مختصر"""
        try:
    ...
    def generate_html_report(self):
        ...
        # أكثر هجوم نجاحاً (على مستوى نوع الهجوم)
        ...
        # تحليل تركيبات المعلمات (attack + params)
        # إنشاء عمود مركب
        self.df_steps["combo_str"] = self.df_steps.apply(
            lambda row: f"{row['attack_type']} -> {self._simplify_params(row['params'])}",
            axis=1,
        )
        combo_stats = self.df_steps.groupby("combo_str")["success"].agg(
            ["sum", "count"]
        )
        combo_stats["success_rate"] = combo_stats["sum"] / combo_stats["count"] * 100
        # الأكثر استغلالاً (أعلى عدد نجاحات)
        most_exploited = combo_stats.sort_values("sum", ascending=False).head(10)
        # الأقل استغلالاً (أقل عدد نجاحات، لكن فقط التي لها على الأقل محاولة واحدة)
        least_exploited = combo_stats.sort_values("sum", ascending=True).head(10)

        # بناء جداول HTML للأكثر والأقل استغلالاً
        most_rows = ""
        ...
        # تجميع البيانات المستخرجة (discovered)
        all_subdomains = set()
        all_ips = set()
        all_mx = set()
        all_ns = set()
        ...
        # بناء قسم البيانات المستخرجة
        discovered_html = ""
        if all_subdomains or all_ips or all_mx or all_ns or all_cnames or total_answers:
            discovered_html = """
            <div style="background:#000000; padding:15px; border-radius:10px; margin:20px 0;">
                <h2 style="color:#2c3e50;"> Extracted Intelligence Summary</h2>
        ...
        <head>
            ...
            <style>
                ...
                .card {{ background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 12px; flex: 1; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
                ...
                th {{ background: #f8f9fc; padding: 12px; text-align: left; }}
        ...
        <div class="container">
            ...
            <div class="table-container">
                <h2> Most Exploited Parameter Combinations (Top 10 by Successes)</h2>
                <table class="min-w-full divide-y divide-gray-200" style="width:100%; border:1px solid #eee;">
            ...
            <div class="table-container">
                <h2> Least Exploited Parameter Combinations (Bottom 10 by Successes)</h2>
                <table class="min-w-full divide-y divide-gray-200" style="width:100%; border:1px solid #eee;">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Rank</th>
    ...
    def generate_report(self, open_browser=True):
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".html", delete=False, dir=self.result_dir) as f:
            f.write(html)
            report_path = f.name
        if open_browser:
